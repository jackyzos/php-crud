...
if (!empty($movieActors) || !empty($movieGenre) ) {
  ...
  public function display_data(){
    ...
    $sql ="SELECT * FROM movies JOIN directors
          ON movies.DirectorID = directors.DirectorID  ORDER BY MovieID DESC";
    $result = $db->query($sql);
    if($result->num_rows > 0){
      $i = 0;
      $array =  array();
...
if (!empty($movieActors) || !empty($movieGenre) ) {
  ...
  public function getMovieByID($id){
    ...
    INNER JOIN moviesgenre
    ON  moviesgenre.MovieID = movies.MovieID
    INNER JOIN genre
    ON  genre.GenreID = moviesgenre.GenreID
    INNER JOIN moviesactors
    ON  moviesactors.MovieID = movies.MovieID
    INNER JOIN actors
